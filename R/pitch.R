...
contrasts(d$condition) = cbind(inf_vs_pol=c(1,-1))
contrasts(d$gender) = cbind(f_vs_m=c(1,-1))
...
## Plot
d_bysubj = na.omit(d) %>%   group_by(subject) %>%   summarise(mean_pitch = mean(pitch))
ggplot(d_bysubj, aes(x=factor(subject), y=mean_pitch)) +   geom_point(size=4, aes(colour = factor(subject)))
