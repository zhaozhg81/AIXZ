...
## First dimension, Admitted vs Rejected
## Second dimension, Male vs female
## Third dimension, departments

male.admit <- apply( UCBAdmissions[,1,],1,sum )[1]
male.reject <- apply( UCBAdmissions[,1,],1,sum )[2]
male.accpt.rate <- male.admit/(male.admit+ male.reject)


female.admit <- apply( UCBAdmissions[,2,],1,sum )[1]
female.reject <- apply( UCBAdmissions[,2,],1,sum )[2]
female.accpt.rate <- female.admit/(female.admit+ female.reject)


female.accpt.dept <- array(0, 6)
male.accpt.dept <- array(0, 6)


for( i in 1:6)
  {
    male.accpt.dept[i] <- UCBAdmissions[1,1,i]/(     UCBAdmissions[1,1,i] + UCBAdmissions[2,1,i])
    female.accpt.dept[i] <- UCBAdmissions[1,2,i]/(     UCBAdmissions[1,2,i] + UCBAdmissions[2,2,i]) 
