...
ceb <- read.table("http://astro.temple.edu/~zhaozhg/Stat8113/data/ceb.txt", header=TRUE)
ceb$y <- floor( ceb$y )
...
##
ceb.pois.2 <- glm( y~educ + res + dur, offset=log(n), family=poisson("log"), data=ceb )
lrtest( ceb.pois.2, ceb.nb )
